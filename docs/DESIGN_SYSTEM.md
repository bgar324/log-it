...
- **The exposed app must recede, and the seam must be drawn.** Two surfaces painted `--bg` meeting at an invisible edge read as one broken screen, and a strip of legible app text beside the nav reads as a second column of half-words. So layer A fogs toward `--bg` at 65% with a `backdrop-blur(7px)` while the drawer is open, and the drawer's `border-r` hairline (canon `--text` 12%) draws the join. The fog is a pseudo-element kept mounted at `opacity: 0` and ramped over the same 280ms as the slide: introduced with the open state instead, it has no previous frame to transition from and snaps to full strength across the whole screen before the app has moved. The bottom bar paints above layer A and carries its own copy of the same fog; its top hairline stays undimmed on purpose, so the one band it shares with the drawer footer stays continuous.
