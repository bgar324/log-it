...
- Week and exercise reordering use the same two-tap move flow, not drag-and-drop. The first tap selects a workout or exercise, and the second chooses its fixed destination while the other items shift. Tapping the selected item again cancels the selection. `Done` applies the draft order and `Cancel` discards it. The split editor and workout logger share the same exercise-reorder component, so their copy, controls, and behavior stay identical.
...
- Destructive row actions live behind an explicit edit mode, not on every row. The split editor exposes `Add exercise` directly, while its day tools menu enters delete mode or opens the two-tap exercise move flow. Deleting is immediate because entering edit mode is the confirmation.
...
- **Reordering uses ordinary buttons.** Week, split-exercise, and logger-exercise move flows all select a source and then a destination without moving focus when the sheet opens.
