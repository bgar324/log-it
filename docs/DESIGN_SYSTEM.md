...
- Reorder lists carry one grab handle, not a handle plus up/down arrows. Pointer drag uses `setPointerCapture` with `[touch-action:none]` on the handle. The week sheet keeps weekdays fixed as destination slots, marks `Today`, and renders the dragged workout as a viewport-fixed preview that follows the finger while the other assignments move around it.
