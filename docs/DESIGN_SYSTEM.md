...
- **Account-targeted exception:** When the PostHog `Ben` flag is true, Split replaces Nutrition in the phone bar and leaves the drawer, while Nutrition leaves the desktop sidebar. Logger set rows omit Reps but keep Weight and Time. A missing, loading, or false flag preserves the standard system.
...
- Week reordering is a two-tap move flow, not drag-and-drop: the first tap selects a workout and the second chooses its fixed weekday destination while the other assignments shift around it. The selected workout can be tapped again to cancel selection; `Done` applies the draft order and `Cancel` discards it. Exercise reordering keeps one grab handle and no up/down arrows.
...
- Destructive row actions live behind an explicit edit mode, not on every row. The split editor exposes `Add exercise` directly, while its day tools menu enters delete mode or opens exercise reordering. Deleting is immediate because entering edit mode is the confirmation.
...
- **Known consequence:** exercise reordering remains pointer-only on its grab handle. Week reordering uses ordinary buttons, so the source and destination work by touch or keyboard without moving focus when the sheet opens.
