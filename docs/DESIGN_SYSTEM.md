...
- **The exposed app becomes a card, not a fogged wall.** Two surfaces painted `--bg` meeting at an invisible edge read as one broken screen. The fix is elevation, not obscuration: while the drawer is open, layer A takes a `--text` 9% veil, a canon 12% hairline edge and a `1.5rem` corner crop with a small band above and below, so the strip left on screen reads as a lit phone screen resting on the drawer. It is deliberately *lighter* than the drawer in dark mode (shaded in light) — a veil toward `--bg` plus a heavy blur was built first and rejected: it smears the app into the background and reads as a modal scrim rather than a second screen. The veil is kept mounted at `opacity: 0` and ramped over the same 280ms as the slide: introduced with the open state instead, it has no previous frame to transition from and snaps to full strength across the whole screen before the app has moved. Crop by covering, never by clipping — see `docs/ARCHITECTURE.md`.
...
- Reorder lists carry one grab handle, not a handle plus up/down arrows. Pointer drag only: `onPointerDown` + `setPointerCapture`, with `[touch-action:none]` on the handle. That utility is load-bearing, not cosmetic — without it a touch drag scrolls the page instead of reordering the list.
...
The authenticated app (`app/dashboard/**`, `app/workouts/**`, `app/exercises/**`, `app/components/app-nav.tsx`) currently ships without an accessibility layer: no `aria-*`, no semantic `role=` values, no `sr-only` announcements, no `.focus()` calls, no focus traps, no keyboard drag paths, no `focus-visible` outlines. This is a record of the current state and the owner's preference on a personal project, not a standard to defend.
...
- **What was kept deliberately, because it is interaction rather than semantics:** `[touch-action:none]` and `[touch-action:manipulation]`, pointer drag (`setPointerCapture`), body scroll locks, `Escape`-to-close, scrim click-to-dismiss, `tabIndex={-1}` on invisible scrim buttons, and the 44px/16px phone minimums.
- **Known consequence:** overlays and reorder controls in the authenticated app are pointer-only. Reordering has no keyboard path at all, since the arrow buttons and the space/arrow handler were the only one. If a keyboard path is ever wanted back, it belongs on the grab handle and must not move focus on open.
