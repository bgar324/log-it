...
What the reveal still needs, and did not ship with, is a boundary. Both layers are painted `--bg`, so the exposed strip of app had no edge and its half-cut headings read as a second column of text next to the nav labels. The first fix was obscuration — a seam hairline on the drawer plus a `--bg` fog and a 7px blur over layer A — and it was rejected on sight: fogging the app toward the page colour smears it into the drawer and reads as a modal scrim, which is exactly the panel-over-app language the layer model exists to avoid.

The shipped fix is elevation. Layer A becomes a card: a `--text` 9% veil (so it reads *lighter* than the drawer in dark and shaded in light), a canon 12% hairline edge, and a `1.5rem` corner crop with a band above and below, sized from the safe-area insets so on a phone it lands in the status-bar and home-indicator dead space. The strip left on screen is then legibly a second phone screen resting on the menu, and no blur is needed to stop it competing — depth does the work that obscuration was doing badly. The card is painted by covering rather than clipping (a `--bg` `box-shadow` ring held at the card's edge by `clip-path`), so layer A's box, scroll position and sticky header are untouched at any content height.
