...
- A user cannot create the same normalized workout type twice on one date. `/workouts/new` redirects to the existing workout when the active split seeds a type already logged for the selected date, and the create service rejects a duplicate that reaches the write boundary. Different workout types may coexist on one date.
...
- Week reordering uses fixed weekday slots and two deliberate taps: choose a workout, then choose its destination day. The other assignments shift around it. Tapping the selected workout again cancels the selection; `Done` applies the draft order and `Cancel` discards it.
