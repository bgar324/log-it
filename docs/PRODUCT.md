...
- Owner sign-out is directly available in the phone bottom bar and desktop sidebar. It respects unsaved-edit and pending-save protection. Profile and Settings retain their header controls.
- A chosen profile photo applies immediately rather than waiting for a separate save.
- Signed-in users land on `/dashboard`.
- The dashboard has seven views: overview, workouts, progress, nutrition, split, profile, and settings. The owner's phone bar provides Home, History, Log, Split, Analysis, Profile, and Sign out. The owner interface has no drawer. Desktop retains its sidebar; unflagged accounts keep the previous interface.
