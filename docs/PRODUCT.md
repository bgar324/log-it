...
- Split exercise rows are borderless name/sets pairs with visible column labels. `Add exercise` is visible in the day editor. Reordering and the explicit delete mode live in the day tools menu; delete removes the exercise immediately because entering that mode is the confirmation. Exercise reordering uses the same two-tap move flow as week and logger reordering: select an exercise, then select its destination position. `Done` applies the draft order and `Cancel` discards it.
