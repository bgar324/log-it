...
- **The card frame** (`navStyles.appCard`) is what makes the exposed part of layer A read as a phone screen resting on the drawer: a viewport-anchored overlay, the last stage child at `z-40`, `pointer-events-none`, translating with layer A and ramping from `opacity: 0` over the same 280ms. It veils layer A with `--text` at 9% (so the card is *lighter* than the drawer in dark and shaded in light), strokes its edge with the canon 12% hairline, and crops it to a `1.5rem` radius.

The crop covers rather than clips: `box-shadow: 0 0 0 6rem var(--bg)` with no offset paints a ring of the page colour outside the rounded rect, filling the corner wedges and the bands above and below, so layer A's own box, scroll position, sticky header and fixed descendants are never touched. `clip-path: inset(-6rem -6rem -6rem 0)` holds that ring at the card's left edge; without it the spread reaches into the drawer and paints over its divider and nav labels (the drawer's content ends at `x=263` against a card edge at `x=280`). The bands are `calc(0.55rem + env(safe-area-inset-*))`, which lands them in the status-bar and home-indicator dead space on a notched phone and shrinks them to a sliver in a browser; measured at 390×844 that leaves 2px of clearance above the header avatar and 5px below the tab labels, and the corner wedges only ever cover the empty corners of those boxes. The overlay must be a stage sibling rather than a layer-A pseudo-element because the bottom bar is also a stage sibling: a pseudo-element on layer A paints underneath the bar and leaves it bright.

Open state rides on a `data-drawer="open" | "closed"` attribute set on all four moving parts (drawer, layer A, bottom bar, card) rather than an appended open class. The open utilities override base values rather than only adding to them, and two utilities from the same family win by emission order in the generated stylesheet, not by attribute order — an appended `opacity-100` cannot be relied on to beat a base `opacity-0`, while a `data-[drawer=open]:` variant wins on specificity.
