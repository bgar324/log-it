...
- **Layer B — the drawer** is the base layer: always mounted, pinned to the left edge, `z-0`, `visibility: hidden` while closed. It holds the second-class sections (Workouts, Progress, Split, Profile), the shared three-segment theme control, and sign out. It carries the seam: a `border-r` hairline at the canon `--text` 12%, which is the only thing separating two surfaces both painted `--bg`. The seam belongs to the drawer because the drawer's edge is the one place it runs unbroken from the status bar to the bottom of the screen — layer A's overlay stops at the bottom bar, which is a stage sibling painted above it.
- **Layer A — the app screen** sits on top at `z-10` with an opaque background. Opening the drawer translates layer A right by `min(17.5rem,78vw)` to reveal layer B underneath, rather than sliding a panel over the app. Layer A also recedes: an always-mounted `::after` overlay tinted toward `--bg` at 65% with `backdrop-blur(7px)`, ramped from `opacity: 0` over the same 280ms as the slide, so the sliver of app left beside the drawer reads as a layer behind instead of a column of half-words competing with the nav. Dismissed by tapping the exposed app surface or pressing Escape; there is no gesture trigger.

Open state rides on a `data-drawer="open" | "closed"` attribute set on all three moving parts (drawer, layer A, bottom bar) rather than an appended open class. The open utilities now override base values rather than only adding to them, and two utilities from the same family win by emission order in the generated stylesheet, not by attribute order — an appended `opacity-100` cannot be relied on to beat a base `opacity-0`, while a `data-[drawer=open]:` variant wins on specificity. The dim and blur are declared as `layerRecede` / `layerRecedeOpen` in `app-nav.styles.ts` and shared by layer A and the bottom bar, since the bar paints above layer A's overlay and needs its own copy.
