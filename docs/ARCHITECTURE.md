...
`instrumentation-client.ts` initializes PostHog. Authenticated client surfaces call `useIdentifyPostHogUser()` with the stable database user ID and profile properties before reading account-targeted flags. The `Ben` flag changes presentation only: it replaces Nutrition with Split in the phone bar, removes the duplicate Split drawer item, removes Nutrition from the desktop sidebar, and hides the logger's Reps field. Direct routes and saved workout data remain available. Missing, loading, or false evaluations keep the standard interface.
